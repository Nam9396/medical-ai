...
from core.embedding import create_index
...
st.title("Pubmed Search And Answer")

st.markdown("""
## Tìm kiếm, phân tích và trả lời câu hỏi
...
**Lưu ý**: Chỉ tải, xử lý và trả lời câu hỏi dựa trên các bài báo từ cơ sở dữ liệu Free Pubmed Central. Ứng dụng không thao tác trên các bài báo Pubmed phải trả phí.     
            
""")

user_question = st.text_input("Nhập câu hỏi", "What is the outcome of pediatric autoimmune encephalitis?")
...
if st.button("Tìm kiếm") and user_question:
    ...
    with st.spinner("Đang tìm kiếm từ cơ sở dữ liệu ...", show_time=True): 
        try: 
            ...
            with st.expander("Bước 2: Tổng quan kết quả tìm kiếm"):
                ...
                st.markdown("Bạn có thể copy và dán các ID bên dưới vào ô tìm kiếm của PMC search.")
                st.markdown(f'{id_string}')  
        except Exception as e: 
            display_general_error(e=e, message="Phát sinh lỗi trong quá trình tìm kiếm từ Pubmed.")
    ...
    with st.spinner("Đang tải và đánh giá các bài báo", show_time=True):
        ...
        with st.expander("Bước 3: Tổng quan các bài báo phù hợp"):
            ...
            for index, article in enumerate(relevant_articles):
                ...
                st.write(f"{index + 1}. {title} - Pubmed ID: {pmid}")
        
    with st.spinner("Đang lập chỉ mục", show_time=True):
        try: 
            vector_store = create_index(store_id="", _docs=articles_chunk_store)
            with st.expander("Bước 4: Lập chỉ mục truy xuất thành công"):
                st.write("Chỉ mục được dùng để truy xuất câu trả lời")
        except Exception as e: 
            display_general_error(e=e, message="Phát sinh lỗi trong quá trình lập chỉ mục tài liệu.")
    ...
    with st.spinner("Trả lời câu hỏi", show_time=True):
        ...
        st.markdown("Yêu cầu mô hình trả lời theo 03 cách khác nhau")
        st.markdown(response["answer"])
        st.markdown("---")
        
        with st.expander("Trích dẫn nguồn tài liệu"):
            for doc in response["context"]:
                metadata_info = f"Tên: {doc.metadata['title']} - Pubmed ID: {doc.metadata['pmid']}"        
    ...
    with st.spinner("Viết bài tổng hợp ngắn", show_time=True):
        ...
        for attempt in range(3):
            try:
                response = synthesize_topic_graph(
                    query=user_question, 
        ...
        st.markdown(response["synthesized_doc"])
        st.markdown("---")    
