...
from graphs.query_rank_graph import query_rank_graph
from components.ui import display_general_error, display_retry_loop_error


st.title("Pubmed Search PMIDs")

st.markdown("""
## Tìm kiếm và đánh giá tự động bài báo khoa học
...
5. Copy paste các ID này vào ô tìm kiếm trên website Pubmed để tra cứu toàn văn.         
...
if st.button("Tìm kiếm") and user_query:
            ...
            try:
                ...
                with st.expander(f"Bước 2: Kết quả tìm kiếm ban đầu (hiển thị 10/{doc_analyse_number})"):
                    for article in articles[:10]:
                        ...
                        st.write(f"PMID: {id} - Tiêu đề: {title}")
                break
            except Exception as e:
    ...
    with st.spinner("Đang phân tích và xếp hạng mức độ liên quan ...", show_time=True):
        try:
            response = query_rank_graph(
                query=user_query, 
                documents=articles
            )
            with st.expander("Bước 3: Các bài báo có liên quan nhất"):
                st.markdown("### Danh sách PMID")
                st.markdown(f'Tổng số PMID: {len(response["relevant_docs_ids"])}')
                ...
                for doc in response["relevant_docs"]:
                    ...
                    with col1:
                        ...
                        st.markdown(f'**PMID:** {doc["content"].metadata["article_ids"]["pubmed"]}')
            ...
            display_general_error(e=e, message="Đã xảy ra lỗi trong quá trình phân tích bài báo.")
